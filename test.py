...
class MyHTTPTest(AsyncHTTPTestCase):
    ...
    def shorten(self):
        # Shorten an URL.
        self.http_client.fetch(self.get_url('/shorten?longUrl=http%3A%2F%2Fwww.familo.net%2Fen%2F%3Fref%3Dhttp%253A%252F%252Ffamilo.net%252F'), self.stop)
        response = self.wait()
    ...
    def shorten_mobile(self):
        # Shorten an URL.
        self.http_client.fetch(self.get_url('/shorten?longUrl=http%3A%2F%2Fwww.familo.net%2Fen%2F%3Fref%3Dhttp%253A%252F%252Ffamilo.net%252F&androidUrl=familonet%3A%2F%2F&androidFallbackUrl=https%3A%2F%2Fplay.google.com%2Fstore%2Fapps%2Fdetails%3Fid%3Dnet.familo.android&iosUrl=familonet%3A%2F%2F&iosFallbackUrl=https%3A%2F%2Fitunes.apple.com%2Fde%2Fapp%2Ffamilonet-die-familien-app%2Fid638696816'), self.stop)
        response = self.wait()
        self.assertIn("json", response.headers.get('Content-Type', ""))
        response.json = json.loads(response.body)
        ...
        self.assertEqual(data.get('android_fallback_url'), 'https://play.google.com/store/apps/details?id=net.familo.android')
        self.assertEqual(data.get('ios_url'), 'familonet://')
        self.assertEqual(data.get('ios_fallback_url'), 'https://itunes.apple.com/de/app/familonet-die-familien-app/id638696816')
        self.assertEqual(data.get('url'), 'http://localhost/' + url_hash)
    ...
    def test_redirection_mobile(self):
        ...
        self.assertIn("https://play.google.com/store/apps/details?id=net.familo.android", response.body)
        ...
        self.assertIn("text/html", response.headers.get('Content-Type', ""))
        self.assertIn("familonet://", response.body)
        self.assertIn("https://itunes.apple.com/de/app/familonet-die-familien-app/id638696816", response.body)
